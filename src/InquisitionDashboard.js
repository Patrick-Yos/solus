...
import {
  // Core Icons
  Skull, Crosshair, Flame, Book, Cpu, Scroll, Terminal, ChevronRight,
  ShieldAlert, Activity, X, Dices, AlertTriangle, Fingerprint, Radio,
  Eye, Target, Zap, Lock, BarChart3, Globe, Wifi, Sword, Radioactive,
  Biohazard, Sparkles, Bug, ShieldCheck, Heart, Brain, Volume2, VolumeX,
  ChevronDown, RotateCw, Gauge, AlertOctagon, Check, AlertCircle, HelpCircle,
  Trophy, Search, Swords, FlameIcon, Trophy as TrophyIcon
} from 'lucide-react';
...
const DecryptText = ({ text, className, chaosLevel = 0 }) => {
        ...
        text.split('').map((letter, index) => {
          if (index < iteration) {
            if (chaosLevel > 70 && Math.random() > 0.7) {
              return corruptedChars[Math.floor(Math.random() * corruptedChars.length)];
            }
            return text[index];
          }
  ...
  return <span className={`${className} ${chaosLevel > 80 ? 'chaos-glitch' : ''}`}>{display}</span>;
};
...
const TacticalAuspex = ({ onClose, chaosLevel }) => {
  ...
  useEffect(() => {
    if (window.anime) {
      window.anime({
        ...
        scale: [0.8, 1], opacity: [0, 1], duration: 400, easing: 'easeOutExpo'
      });
    }
    ...
    const initBox = async () => {
      try {
        const box = new DiceBox({
          ...
          theme: 'rust', themeColor: '#800000', scale: 6,
        });
  ...
  const rollDice = async (customNotation = null) => {
    ...
    setRolling(true); setResult(null); diceBoxRef.current.clear();
    ...
    try {
      ...
      let total = Array.isArray(res) ? res.reduce((acc, r) => acc + r.value, 0) : res.value;
    ...
    <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center font-tech p-4 backdrop-blur-sm">
      <div className="auspex-container relative w-full max-w-5xl h-[80vh] bg-zinc-900 border-4 border-[#c5a059] flex flex-col shadow-[0_0_50px_rgba(197,160,89,0.3)] opacity-0">
        <div className="bg-red-950 p-3 flex justify-between items-center border-b-4 border-[#c5a059]">
          ...
          <button onClick={onClose} className="hover:rotate-90 transition-transform"><X className="text-red-500 hover:text-white w-8 h-8"/></button>
        </div>
        ...
        <div className="flex-1 bg-black relative overflow-hidden">
          ...
          {result !== null && !rolling && (
             <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20 pointer-events-none">
                <div className={`bg-black/90 border-2 p-8 text-center ${
                  result.critical ? 'border-green-500' : 'border-red-500'
                }`}>
                   <div className={`text-8xl font-bold font-mono drop-shadow-[0_0_15px_rgba(51,255,0,0.8)] ${
                     result.critical ? 'text-green-400' : 'text-red-400'
                   }`}>
                     {result.total}
                   </div>
                   {result.critical && (
                     <div className="text-green-500 font-tech text-sm mt-2 animate-pulse">CRITICAL HIT</div>
                   )}
                </div>
             </div>
          )}
          ...
          {rolling && (
            <div className="absolute top-4 left-4 z-20 text-yellow-500 font-tech text-sm flex items-center gap-2">
              <RotateCw className="w-4 h-4 animate-spin" /> CALCULATING EMPEROR'S WILL
            </div>
          )}
        </div>
        
        <div className="bg-zinc-950 p-2 border-t border-[#333]">
          <div className="font-tech text-[10px] text-zinc-500 mb-1">RECENT ROLLS:</div>
          <div className="flex gap-2 overflow-x-auto">
            {diceLog.map((log, i) => (
              <div 
                key={i} 
                className={`px-2 py-1 bg-[#222] border text-[9px] whitespace-nowrap ${
                  log.critical ? 'border-green-500 text-green-400' : 'border-[#444] text-zinc-400'
                }`}
              >
...
const WeaponSelector = ({ selectedWeapon, onSelectWeapon, chaosLevel }) => {
  const [weapons, setWeapons] = useState([
    { name: 'BOLT PISTOL', dmg: '2d6+3', range: '24"', type: 'pistol', icon: Crosshair, color: 'text-red-500', ammo: 12, maxAmmo: 12, cooldown: 0, maxCooldown: 0 },
    { name: 'CHAINSWORD', dmg: '1d10+5', range: 'Melee', type: 'melee', icon: Sword, color: 'text-[#c5a059]', ammo: Infinity, maxAmmo: Infinity, cooldown: 0, maxCooldown: 0 },
    { name: 'PLASMA GUN', dmg: '3d8', range: '36"', type: 'rapid', icon: Zap, color: 'text-cyan-400', ammo: 3, maxAmmo: 3, cooldown: 1, maxCooldown: 3 },
    { name: 'FLAMER', dmg: '2d10', range: 'Template', type: 'template', icon: Flame, color: 'text-orange-400', ammo: 6, maxAmmo: 6, cooldown: 2, maxCooldown: 4 },
    { name: 'MELTA', dmg: '4d10', range: '12"', type: 'melta', icon: Radioactive, color: 'text-yellow-400', ammo: 2, maxAmmo: 2, cooldown: 3, maxCooldown: 5 },
    { name: 'PSYKER STAFF', dmg: '2d8+WP', range: '48"', type: 'psyker', icon: Sparkles, color: 'text-purple-400', ammo: 5, maxAmmo: 5, cooldown: 0, maxCooldown: 0 }
  ]);
        ...
        {weapons.map((w, i) => {
          ...
          return (
            <div 
              ...
              title={`${w.name} - AMMO: ${w.ammo}/${w.maxAmmo}${w.cooldown > 0 ? ` - COOLDOWN: ${w.cooldown}s` : ''}`}
            >
              <Icon className={`w-5 h-5 ${w.color} mb-1 ${!isReady ? 'animate-pulse' : ''}`} />
              <div className="font-tech text-[9px] text-zinc-300">{w.name}</div>
              <div className="font-tech text-[8px] text-green-500">{w.dmg}</div>
              
              {w.ammo !== Infinity && (
                <div className={`absolute top-1 right-1 w-4 h-4 rounded-full text-[7px] flex items-center justify-center ${
                  w.ammo > 0 ? 'bg-green-600 text-black' : 'bg-red-600 text-white'
                }`}>
                  {w.ammo}
                </div>
              )}
              ...
              {w.cooldown > 0 && (
                <div className="absolute bottom-0 left-0 w-full h-1 bg-[#333]">
                  <div className="h-full bg-red-500" style={{ width: `${(w.cooldown / (w.maxCooldown || 1)) * 100}%` }}></div>
                </div>
              )}
              
              {isSelected && <div className="absolute inset-0 border-2 border-green-500 animate-pulse pointer-events-none"></div>}
            </div>
          );
        })}
      </div>
...
const EnemyTacticalDisplay = ({ enemies, onEngage, chaosLevel, onPurge }) => {
  ...
  const enemyTypes = [
    { name: 'CULTIST', hp: 10, threat: 1, icon: Bug, color: 'text-orange-400', weakTo: 'FLAMER', type: 'mortal' },
    { name: 'CHAOS MARINE', hp: 25, threat: 3, icon: Skull, color: 'text-red-500', weakTo: 'MELTA', type: 'heretic' },
    { name: 'DAEMONETTE', hp: 20, threat: 2, icon: Heart, color: 'text-pink-400', weakTo: 'PSYKER STAFF', type: 'daemonic' },
    { name: 'PLAGUEBEARER', hp: 30, threat: 2, icon: Radioactive, color: 'text-green-400', weakTo: 'PLASMA GUN', type: 'daemonic' },
    { name: 'RUBRIC MARINE', hp: 35, threat: 4, icon: ShieldAlert, color: 'text-purple-400', weakTo: 'BOLT PISTOL', type: 'heretic' },
    { name: 'WARP SPAWN', hp: 50, threat: 5, icon: Biohazard, color: 'text-purple-500', weakTo: 'EXTERMINATUS', type: 'warp' }
  ];

  const spawnEnemy = () => {
    const enemy = enemyTypes[Math.floor(Math.random() * enemyTypes.length)];
    const newEnemy = { ...enemy, id: Date.now(), currentHp: enemy.hp, priority: Math.random() > 0.7 };
  ...
  return (
    <div className="bg-[#111] border-2 border-[#5a2e2e] p-4">
      ...
      <div className="space-y-2 max-h-56 overflow-y-auto custom-scrollbar">
        {enemies.length === 0 ? (
          <div className="text-[10px] text-zinc-500 font-tech text-center py-4">NO THREATS DETECTED</div>
        ) : (
          enemies.map((e) => {
            const Icon = e.icon;
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2">
                  <Icon className={`w-4 h-4 ${e.color} ${e.priority ? 'animate-pulse' : ''}`} />
                  <div>
                    <div className="font-tech text-[11px]">{e.name}</div>
                    <div className="font-tech text-[9px] text-zinc-500">THREAT: {e.threat}</div>
                  </div>
                </div>
                
                <div className="flex items-center gap-2">
                  <div className="font-tech text-[10px] text-red-500">{e.currentHp}/{e.hp}</div>
                  <div className={`w-14 bg-[#333] h-1 border border-[#444] relative`}>
                    <div 
...
const MachineSpiritConsole = ({ satisfaction, setSatisfaction }) => {
  ...
  return (
    <div className="prayer-target bg-[#111] border-2 border-[#333] p-4 relative">
      <div className="absolute top-2 right-2"><Cpu className="w-5 h-5 text-[#c5a059]" /></div>
      ...
      <div className="font-tech text-[11px] text-zinc-400 mb-3">
        Satisfaction: {satisfaction}% {favor > 0 && <span className="text-green-400">(+{favor} favor)</span>}
      </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        >
          {praying ? 'PRAYING...' : <><Sparkles className="w-3 h-3 inline mr-1" /> RECITE CANTICLE</>}
        </button>
      </div>
...
const ChaosMeter = ({ level, setLevel }) => {
  ...
  return (
    <div className="chaos-container bg-[#111] border-2 border-[#333] p-4 relative overflow-hidden">
      <div className="absolute top-2 right-2"><Biohazard className="w-5 h-5 text-purple-500" /></div>
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        >
          <Sparkles className="w-3 h-3" /> CLEANSE {streak > 0 && `(${streak}x)`}
        </button>
      </div>
      
      {level > 80 && (
        <div className="mt-2 font-tech text-[10px] text-purple-400 animate-pulse flex items-center gap-1">
          <AlertTriangle className="w-3 h-3" /> WARNING: WARP SIGNATURES DETECTED
        </div>
      )}
    </div>
  );
    ...
    <div className="relative w-full h-64 border border-[#5a2e2e] bg-black overflow-hidden flex items-center justify-center group">
      <div className="planet-container absolute inset-0">
        ...
        <div className="w-40 h-40 rounded-full bg-[#1a0a0a] shadow-[inset_-10px_-10px_20px_rgba(0,0,0,0.8),0_0_20px_rgba(197,160,89,0.2)] relative overflow-hidden border border-[#c5a059]/30">
          ...
          {damage > 0 && (
            <div className="absolute inset-0 bg-red-900/20" style={{ opacity: damage / 100 }}></div>
          )}
        ...
        <div className="absolute top-4 left-4 text-[10px] font-tech text-red-500">
          COORD: 44.21.99<br/>STATUS: {damage > 50 ? 'CRITICAL' : 'HERESY DETECTED'}
        </div>
...
const InquisitionDashboard = ({ onNavigate }) => {
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      ...
      timeline
        .add({ targets: '.main-interface', scaleY: [0.01, 1], opacity: [0, 1], duration: 800 })
        .add({ targets: '.scanlines', opacity: [0, 1], duration: 1000 }, '-=400')
        .add({ targets: '.boot-sequence', opacity: [0, 1], translateX: [-20, 0], duration: 600 }, '-=200')
        .finished.then(() => {
    ...
    const eventTimer = setInterval(() => {
      const events = [
        ...
        { msg: "Heretical transmission intercepted...", level: 15, type: 'warning' },
  ...
  const handleExterminatus = () => {
    if (exterminatus || !animeReady || favor < 3) {
      if (favor < 3) Anima.notify('REQUIRES 3 FAVOR TO INVOKE EXTERMINATUS', 'error');
      return;
    }
    ...
    timeline
      .add({ targets: 'body', filter: ['brightness(1) hue-rotate(0deg)', 'brightness(0.3) hue-rotate(-60deg) saturate(200%)'], duration: 500 })
      .add({ targets: '.main-interface', translateX: () => window.anime.random(-15, 15), translateY: () => window.anime.random(-15, 15), duration: 3000, easing: 'linear', direction: 'alternate', loop: true }, '-=500')
      .add({ targets: '.exterminatus-overlay', opacity: [0, 1], duration: 100 }, '-=3000')
      .add({ targets: '.countdown-digit', scale: [0, 1], opacity: [0, 1], duration: 500, color: ['#c5a059', '#ff0000'] }, '-=2900')
      .add({ targets: '.blood-splatter', opacity: [0, 0.4], duration: 2000 }, '-=2500');
  };

  const addLog = (msg) => {
    const timestamp = new Date().toLocaleTimeString('en-US', {hour12: false});
    const imperialDate = `${Math.floor(Math.random() * 9) + 1}.${Math.floor(Math.random() * 999)}.${Math.floor(Math.random() * 999)}.M3`;
    setLogs(prev => [...prev.slice(-5), `[${timestamp}] [${imperialDate}] > ${msg}`]);
  };
  ...
  const showShortcuts = () => {
    Anima.notify('SHORTCUTS: [1,2,3,4]=Tabs [SPACE]=Exterminatus [R]=Reset', 'info');
  };
  ...
  return (
    <div className={`min-h-screen bg-[#050505] text-[#c5a059] relative overflow-x-hidden selection:bg-red-900 selection:text-white ${
      ...
      {mode === 'AUSPEX' && <div className="fixed inset-0 z-30 pointer-events-none bg-green-500/10 mix-blend-overlay"></div>}
      <div className={`scanlines ${chaosLevel > 80 ? 'chaos-glitch' : ''}`}></div>
      
      <div className={`blood-splatter fixed inset-0 z-25 pointer-events-none ${exterminatus ? 'blood-active' : ''}`}></div>
        ...
        <div className="exterminatus-overlay fixed inset-0 z-[200] bg-red-900/50 flex items-center justify-center pointer-events-none opacity-0">
          <div className="border-y-8 border-red-600 w-full bg-black/90 py-24 text-center backdrop-blur-sm">
            ...
            <div className="text-4xl text-red-500 font-tech tracking-[1em] animate-pulse">
              {exterminatusCountdown > 0 ? 'EXTERMINATUS IN PROGRESS' : 'THE EMPEROR\'S WRIT'}
            </div>
          </div>
        </div>
      )}
         ...
         <div className="flex items-center gap-4">
             ...
             <div>
               ...
               <div className="flex gap-3 text-[10px] font-tech text-red-500 font-bold tracking-[0.2em] uppercase mt-2">
                 <ShieldAlert className="w-3 h-3" /> SECTOR CALIXIS // CLEARANCE MAGENTA // GRADE: ULTRAVIOLET
               </div>
               <div className="font-tech text-[9px] text-zinc-500 mt-1">
                 {new Date().toLocaleDateString()} - IMPERIAL DATE: 0{Math.floor(Math.random() * 9) + 1}.{Math.floor(Math.random() * 999)}.M3
               </div>
             </div>
         </div>
         ...
         <div className="flex items-center gap-3">
           ...
           <button 
             onClick={() => {
               ...
               addLog(audioMuted ? "VOX-CHANNELS UNMUTED" : "VOX-CHANNELS MUTED");
               Anima.notify(audioMuted ? 'SOUND ON' : 'SOUND OFF', 'info');
             }}
             className="p-2 bg-[#111] border border-[#333] hover:border-[#c5a059] transition-all btn-interactive"
           >
             {audioMuted ? <VolumeX className="w-5 h-5 text-zinc-500" /> : <Volume2 className="w-5 h-5 text-[#c5a059]" />}
           </button>
           
           <button onClick={onNavigate} className="group px-6 py-2 bg-[#2a0f0f] border border-[#5a2e2e] hover:bg-red-900 hover:border-red-500 transition-all flex items-center gap-2 shadow-lg btn-interactive main-interface">
            <div className="text-right hidden md:block">
              <div className="text-[10px] text-red-400 font-tech uppercase">System Override</div>
              <div className="font-gothic font-bold text-xs text-[#c5a059]">RETURN TO SYNDICATE</div>
            </div>
      ...
      <main className="main-interface relative z-10 max-w-7xl mx-auto p-4 md:p-8 crt-flicker" ref={containerRef}>
        
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-8">
            <button onClick={() => { 
              const newMode = mode === 'STANDARD' ? 'AUSPEX' : 'STANDARD';
              setMode(newMode);
              addLog(`VISUAL MODE: ${newMode === 'AUSPEX' ? 'NIGHTSIGHT ACTIVATED' : 'STANDARD VISION'}`);
              Anima.notify(newMode === 'AUSPEX' ? 'AUSPEX MODE ON' : 'AUSPEX MODE OFF', 'info');
            }} 
               className={`p-3 border transition-all flex flex-col items-center justify-center gap-2 btn-interactive ${
                 mode === 'AUSPEX' ? 'bg-green-900/20 border-green-500 text-green-500' : 'bg-[#111] border-[#c5a059] hover:bg-[#c5a059] hover:text-black'
               }`}>
               <Eye className="w-6 h-6" /> 
               <span className="font-gothic font-bold text-xs">{mode === 'AUSPEX' ? 'AUSPEX ON' : 'AUSPEX OFF'}</span>
            </button>
            ...
            <button onClick={() => { 
              ...
              Anima.notify('DICE ROLLER ACTIVATED', 'info');
               ...
               <Dices className="w-6 h-6" /> <span className="font-gothic font-bold text-xs">AUSPEX</span>
            </button>
            ...
            <button onClick={() => { 
              ...
              Anima.notify('RETINUE ROSTER LOADED', 'info');
               ...
               className={`p-3 border transition-all flex flex-col items-center justify-center gap-2 btn-interactive ${
                 tab === 'SQUAD' ? 'bg-[#c5a059] text-black border-[#c5a059]' : 'bg-[#111] border-[#c5a059] text-[#c5a059]'
               }`}>
               <Fingerprint className="w-6 h-6" /> <span className="font-gothic font-bold text-xs">RETINUE</span>
            </button>
            ...
            <button onClick={() => { 
              ...
              Anima.notify('MISSION PARAMETERS LOADED', 'info');
               ...
               className={`p-3 border transition-all flex flex-col items-center justify-center gap-2 btn-interactive ${
                 tab === 'MISSION' ? 'bg-[#c5a059] text-black border-[#c5a059]' : 'bg-[#111] border-[#c5a059] text-[#c5a059]'
               }`}>
               <Target className="w-6 h-6" /> <span className="font-gothic font-bold text-xs">ASSAULT</span>
            </button>
            ...
            <button onClick={() => { 
              ...
              Anima.notify('ARSENAL ONLINE', 'info');
               ...
               className={`p-3 border transition-all flex flex-col items-center justify-center gap-2 btn-interactive ${
                 tab === 'ARSENAL' ? 'bg-[#c5a059] text-black border-[#c5a059]' : 'bg-[#111] border-[#c5a059] text-[#c5a059]'
               }`}>
               <Sword className="w-6 h-6" /> <span className="font-gothic font-bold text-xs">ARSENAL</span>
            </button>
            
            <button onClick={() => { 
              setTab('THREATS'); 
              addLog("THREAT MATRIX: ANALYZING XENOS/HERETIC/DAEMONIC SIGNATURES");
              Anima.notify('THREAT MATRIX ACTIVE', 'info');
            }} 
               className={`p-3 border transition-all flex flex-col items-center justify-center gap-2 btn-interactive ${
                 tab === 'THREATS' ? 'bg-[#c5a059] text-black border-[#c5a059]' : 'bg-[#111] border-[#c5a059] text-[#c5a059]'
               }`}>
               <Bug className="w-6 h-6" /> <span className="font-gothic font-bold text-xs">THREATS</span>
            </button>
        </div>
        ...
        <div className="grid lg:grid-cols-4 gap-6">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 tab-content">
                  {retinue.map((m, i) => {
                    ...
                    return (
                      ...
                      >
                        <div className="purity-seal">{['PURITY', 'FAITH', 'DUTY', 'HONOR', 'VIGILANCE', 'JUSTICE'][i]}</div>
                        <Icon className="absolute bottom-2 left-2 w-32 h-32 opacity-5 pointer-events-none" />
                        <div className="absolute top-2 right-2 flex gap-1">
                          <div className={`w-2 h-2 rounded-full ${
                            m.status === 'OPERATIONAL' || m.status === 'BLESSED' ? 'bg-green-500' : 
                            m.status === 'UNSTABLE' ? 'bg-yellow-500' : 'bg-red-500'
                          } animate-pulse`}></div>
                        </div>
                        ...
                        <div className="relative z-10">
                          ...
                          <p className={`text-xs ${m.factionColor} font-tech tracking-wider mb-3 flex items-center gap-1`}>
                          ...
                          <div className="grid grid-cols-2 gap-2 mt-3">
                            <button 
                              onClick={(e) => {
                                ...
                                Anima.pulse(`.operative-card:nth-child(${i + 1})`);
                   ...
                   <div className="border border-[#c5a059]/30 p-6 h-full relative overflow-hidden flex flex-col gap-6">
                       <div className="flex justify-between items-start">
                           <h2 className="text-3xl font-gothic font-bold text-[#c5a059]">
                             <DecryptText text="PLANETARY ASSAULT: OPERATION FINAL JUDGEMENT" chaosLevel={chaosLevel} />
                           </h2>
                           <div className="text-right font-tech text-xs text-red-500">
                             <div className="mb-2">THREAT LEVEL: <span className="animate-pulse">EXTREMIS</span></div>
                             <div>POPULATION: 4.2 BILLION <span className="text-zinc-400">(EXPENDABLE)</span></div>
                           </div>
                       </div>
                       ...
                       <div className="grid md:grid-cols-2 gap-6 flex-1">
                           <PlanetVisualizer 
                             ...
                             onOrbitalStrike={() => setPlanetaryDamage(prev => Math.min(100, prev + 5))}
                           />
                           <div className="space-y-4 font-tech text-sm text-zinc-300">
                               <div className="p-4 bg-[#050505] border border-[#333] hover:border-[#c5a059] transition-all">
                                   <div className="text-[#c5a059] font-bold mb-2 flex items-center gap-2">
                                     <Wifi className="w-4 h-4 animate-pulse" /> INTERCEPTED VOX-CAST
                                   </div>
                                   <p className="italic opacity-70">"...they came from the skies... shadows with burning eyes... Emperor save us..."</p>
                                   <p className="text-red-500 text-[10px] mt-2">- Last transmission from Hive Secundus</p>
                               </div>
                               ...
                               <div className="p-4 bg-[#050505] border border-[#333] space-y-3">
                                 ...
                                 <div className="space-y-2 text-[11px]">
                                   <div className="flex items-center gap-2 hover:text-green-500 transition-all cursor-pointer" onClick={() => setPlanetaryDamage(Math.min(100, planetaryDamage + 10))}>
                                     <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div> LZ ALPHA SECURED
                                   </div>
                                   <div className="flex items-center gap-2">
                                     <div className="w-2 h-2 bg-yellow-500 rounded-full"></div> HIVE CITY 7 - CONTESTED
                                   </div>
                                   <div className="flex items-center gap-2">
                                     <div className="w-2 h-2 bg-red-500 rounded-full animate-pulse"></div> MANUFACTORUM - HERETIC CONTROL
                                   </div>
                                   <div className="flex items-center gap-2">
                                     <div className="w-2 h-2 bg-purple-500 rounded-full animate-pulse"></div> WARP ANOMALY - DAEMON PRESENCE
                                   </div>
                                 </div>
                               </div>
                               ...
                               <div className="p-4 bg-[#050505] border border-[#333]">
                                 ...
                                 <div className="grid grid-cols-2 gap-2 text-[10px] text-zinc-400">
                                   <div className="hover:text-[#c5a059] transition-all">• Thunderhawks: 3</div>
                                   <div className="hover:text-[#c5a059] transition-all">• Leman Russ: 12</div>
                                   <div className="hover:text-[#c5a059] transition-all">• Astartes: 1 Squad</div>
                                   <div className="hover:text-[#c5a059] transition-all">• Orbitals: ONLINE</div>
                                 </div>
                               </div>
                           </div>
                       </div>
              ...
              {tab === 'ARSENAL' && (
                <div className="grid md:grid-cols-2 gap-6 tab-content">
                  ...
                  <div className="bg-[#111] border-2 border-[#5a4a3a] p-4">
                    ...
                    <div className="font-tech text-[24px] text-center text-green-400">{favor}</div>
                    <div className="font-tech text-[9px] text-zinc-500 text-center mt-1">Omnsissiah's Blessing</div>
                  </div>
                </div>
              )}
                  ...
                  <div className="space-y-4">
                    <div className="bg-[#111] border-2 border-red-900 p-4">
                      <h3 className="font-gothic font-bold text-red-500 mb-3">
                        <AlertOctagon className="w-4 h-4 inline mr-2" /> PRIMARY TARGET
                      </h3>
                      <div className="font-tech text-[12px] text-red-400 space-y-1">
                        <div className="font-gothic text-sm">DEMON PRINCE KOR'LAETH</div>
                        <div className="text-[10px] text-zinc-400">Location: Hive Primus Cathedral</div>
            ...
            <div className="lg:col-span-1 space-y-6">
                ...
                <div className="bg-[#111] border-2 border-[#5a2e2e] p-4 font-tech text-xs space-y-4">
                    ...
                    <div className="space-y-2 text-[11px]">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className={satisfaction > 50 ? 'text-green-500' : 'text-red-500'}>
                            {satisfaction > 50 ? 'APPEASED' : 'UNQUIET'}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span>WARP DRIVE</span>
                          <span className={chaosLevel > 80 ? 'text-red-500 animate-pulse' : 'text-yellow-500'}>
                ...
                <div className="bg-black border-2 border-[#333] p-4 font-tech text-xs text-green-500 h-64 overflow-hidden relative shadow-inner">
                    ...
                    <div className="absolute top-2 left-2 font-tech text-[10px] text-green-700">COMBAT LOG v2.41</div>
                    <div className="space-y-1 mt-6">
                        {combatLogs.map((log, i) => (
                          <div key={i} className="opacity-90 hover:opacity-100 transition-all hover:text-green-400">
                            {log}
                          </div>
                        ))}
                        {combatLogs.length === 0 && (
                          <div className="text-green-700 italic">NO RECENT COMBAT ACTIVITY</div>
                        )}
                ...
                <div className="bg-[#1a0505] border-2 border-red-900 p-6 flex flex-col items-center justify-center text-center relative overflow-hidden shadow-[0_0_30px_rgba(255,0,0,0.1)] group">
                    ...
                    <div className="purity-seal absolute top-2 left-2 text-[8px]">PURITY</div>
                    <div className="purity-seal absolute bottom-2 right-2 text-[8px]">FAITH</div>
                    ...
                    <p className="font-tech text-[10px] text-zinc-400 mb-3">Authorization: Lord Inquisitor Only</p>
                    <button onClick={handleExterminatus} disabled={exterminatus || favor < 3}
                        ...
                        <span className="relative z-10">{exterminatus ? "ARMED..." : "EXTERMINATUS"}</span>
                    ...
                    {exterminatus && (
                      <div className="mt-2 font-tech text-[10px] text-red-400 animate-pulse">
                        <Gauge className="w-3 h-3 inline mr-1" /> ARMAGEDDON PATTERN ACTIVATED
                      </div>
                    )}
                </div>
      ...
      <footer className="relative z-10 bg-black border-t-4 border-[#5a2e2e] py-6 text-center mt-12">
        <p className="font-gothic text-[#c5a059] text-xs tracking-[0.3em] opacity-60">++ IMPERIUM OF MAN • HOLY GOD-EMPEROR • TERRA ++</p>
        <p className="font-tech text-[9px] text-zinc-600 mt-1">"Innocence proves nothing" - Inquisitorial Proverb</p>
      </footer>
    </div>
  );
};
